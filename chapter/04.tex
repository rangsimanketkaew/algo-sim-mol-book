...
%----------------------------------------
\section{กรณีศึกษา: โปรแกรม CP2K}
%----------------------------------------

พลังงานรวมของโมเลกุลที่คำนวณด้วยวิธี Density Functional Theory (DFT) โดยการใช้ Gaussian Plane Wave ในโปรแกรม 
CP2K นั้นมีสมการดังต่อไปนี้
...
\begin{align}
  ...
  &+ \sum_{\mu \nu} P^{\mu \nu}\left\langle\varphi_\mu(\boldsymbol{r})\left|V_{\mathrm{loc}}^{\mathrm{PP}}(r)\right| \varphi_\nu(\boldsymbol{r})\right\rangle \nonumber \\
  &+ \sum_{\mu \nu} P^{\mu \nu}\left\langle\varphi_\mu(\boldsymbol{r})\left|V_{\mathrm{nl}}^{\mathrm{PP}}\left(\boldsymbol{r}, \boldsymbol{r}^{\prime}\right)\right| \varphi_\nu\left(\boldsymbol{r}^{\prime}\right)\right\rangle \nonumber \\ 
  &+ 2 \pi \Omega \sum_G \frac{\widetilde{n}^*(\boldsymbol{G}) \tilde{n}(\boldsymbol{G})}{\boldsymbol{G}^2}+\int e^{\mathrm{xc}}(\boldsymbol{r}) \mathrm{d} \boldsymbol{r} \nonumber \\ 
